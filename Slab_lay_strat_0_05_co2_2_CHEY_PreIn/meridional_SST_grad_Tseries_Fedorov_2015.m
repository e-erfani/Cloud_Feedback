...
cases = {            'co2_Slab_CHEY_PreIn',...
 'neg_Slab_lay_strat_0_2_co2_2_CHEY_PreIn',...
'neg_Slab_lay_strat_0_15_co2_2_CHEY_PreIn',...
 'neg_Slab_lay_strat_0_1_co2_2_CHEY_PreIn',...
'neg_Slab_lay_strat_0_05_co2_2_CHEY_PreIn',...
    'Slab_lay_strat_0_05_co2_2_CHEY_PreIn',...
     'Slab_lay_strat_0_1_co2_2_CHEY_PreIn',...
    'Slab_lay_strat_0_15_co2_2_CHEY_PreIn',...
     'Slab_lay_strat_0_2_co2_2_CHEY_PreIn'};
...
for i = 1:9
  ...
  for tt1=1:length(aa)
    ...
    % set extratropical Pacific to 2 
      ...
      grad_SST_Tseries(tt1,i) = tropical_SST - extratropical_SST;
...
%%%%%%
time = 1:length(aa) ;
       fig_name = strcat('merid_grad_SST_Tseries_Fedorov_2015');%,num2str(tt));
        fig_dum = figure(1);
...
h9 = plot(time,squeeze(grad_SST_Tseries(:,9)),'g','linewidth',3) ; hold on
    ...
    ylabel('Meridional SST gradient (\circC)','fontsize',23,'fontweight','bold');
   hleg1 = legend([h1 h2 h3 h4 h5 h6 h7 h8 h9],'12',...
...
%%%%%%
time = 1:length(aa) ;
       fig_name = strcat('merid_grad_SST_Tseries_Fedorov_2015_smooth_10');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
