...
clear mask B
      ...
      grad_SST_Tseries(tt1,i) = west_SST-east_SST; 
...
%%%%%%
time = 1:length(aa) ;
       fig_name = strcat('zonal_grad_SST_Tseries_Fedorov_2015');%,num2str(tt));
        fig_dum = figure(1);
...
%%%%%%
time = 1:length(aa) ;
       fig_name = strcat('zonal_grad_SST_Tseries_Fedorov_2015_smooth_10');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
