...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)'), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_S_d_bias_annual_FILE_CAM_coef_NEW_CERES');
        fig_dum = figure(100);
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)'), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_S_d_bias_diff_annual_CAM_coef_NEW_CERES');%,num2str(tt));
        fig_dum = figure(144);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
dbstop
 ...
 nccreate('Bias_S_d_daily_CAM_coef_slab.nc','Bias_S_d_daily','Dimensions',{'lon',96,'lat',48,'day',365},'Format','classic');
 nccreate('Bias_S_d_daily_CAM_coef_slab.nc','lat','Dimensions',{'lon',96,'lat',48},'Format','classic');
 nccreate('Bias_S_d_daily_CAM_coef_slab.nc','lon','Dimensions',{'lon',96,'lat',48},'Format','classic'); 
  ncwrite('Bias_S_d_daily_CAM_coef_slab.nc','Bias_S_d_daily',Bias_S_d_2_dail_mea);
  ncwrite('Bias_S_d_daily_CAM_coef_slab.nc','lat',lat_msh');
  ncwrite('Bias_S_d_daily_CAM_coef_slab.nc','lon',lon_msh');

%%%%%%%

Test_SW = ncread('Bias_S_d_daily_CAM_coef_slab.nc','Bias_S_d_daily') ;
Test_lat = ncread('Bias_S_d_daily_CAM_coef_slab.nc','lat') ;
Test_lon = ncread('Bias_S_d_daily_CAM_coef_slab.nc','lon') ;
...
[Z_adjust(:,:,1),refvec_bias(:,:,1)] = geoloc2grid(squeeze(lat_T31(:,:,1)),squeeze(lon_T31(:,:,1)),squeeze(SST_T31(:,:,1)), cellsize);   
 ...
 lonlim = [1 360];
        ...
        fig_name = strcat('SW_Surf_d_bias_annual_fromFILE_CAM_CAMgrid_diff_NEW_CERES');%,num2str(tt));
        fig_dum = figure(266);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
