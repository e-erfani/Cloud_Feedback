...
for tt1=1:length(aa1)
    filename1=aa1(tt1,1).name;
    fsds =ncread(filename1,'FSDS'); % 
%    sst(sst > 1E4) = NaN ;
%    sst(sst < 100) = NaN ;
    fsds_all(:,:,tt1) = fsds ;

    II=find(isnan(fsds_all(:,:,tt1))==1);
    GW2 = GW ;
    GW2(II)=nan;
    fsds_Tseries_ctrl(tt1) = nansum(nansum(GW2 .* fsds_all(:,:,tt1),1),2) ./ nansum(nansum(GW2,1),2) ;
end

%%%%%%
time = 1:100 ;
       fig_name = strcat('FSDS_TIME_SERIES_GLOBAL_compare');%,num2str(tt));
        fig_dum = figure(3);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.3,0.3,12,9]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
h1 = plot(time,squeeze(fsds_Tseries_ctrl(1:100)),'k','linewidth',1.5) ;
%hold on
...
%h6 = plot(time,squeeze(TOA_imbalance_0_5(1:600)),'c','linewidth',1.5) ;
    ...
    ylabel('Global FSDS (Wm^-^2)','fontsize',23,'fontweight','bold');
