...
%%%%%%%
       fig_name = strcat('zonal_U_867_FCM_anomaly_1800');%,num2str(tt));
        fig_dum = figure(1);
...
for j = 6:9
    ...
    ylabel('867 hPa zonal wind anomaly (m s^-^1)','fontsize',23,'fontweight','bold');
xlim([-90 90])
ylim([-1.6 1.1])
...
%%%%
       fig_name = strcat('zonal_U_867_SOM_anomaly');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
