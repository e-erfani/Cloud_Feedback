...
for i = 1:10
    fname = aa(i,1).name;
    grad_SST = get_east_west_grad_SSTequ_CESM(fname);
...
elseif (tt==10);load(aa(tt,1).name); sst = sst_CO_sl_ps_0_2; end
 ...
 sst = double(sst');
     grad_SST_SOM = get_east_west_grad_SSTequ_CESM_SOM(sst,lat_msh_CAM,lon_msh_CAM,GW);
...
%%%%%%%
       fig_name = strcat('zonal_grad_vs_GSST_Fedorov_15');%,num2str(tt));
        fig_dum = figure(1);
