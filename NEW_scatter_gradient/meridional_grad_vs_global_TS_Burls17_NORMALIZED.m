...
%%%%%%%
       fig_name = strcat('meridional_grad_vs_global_Burls2017_NORMALIZED');%,num2str(tt));
        fig_dum = figure(1);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
      
    h1=plot(sst_fcm(1:9),Pac_meri_grad_UO_fcm ./ sst_fcm(1:9),'o','markers',13); hold on
    set(h1,'MarkerEdgeColor','k','MarkerFaceColor','b','LineWidth',1)

    h2=plot(sst_som(1:9),dSST_m ./ sst_som(1:9),'o','markers',13); hold on
    set(h2,'MarkerEdgeColor','k','MarkerFaceColor','r','LineWidth',1)

  x = xlabel('Global SST (\circC)','fontsize',23,'fontweight','bold');
    ylabel('Normalized \DeltaT_m_e_r_i_d_i_o_n_a_l','fontsize',23,'fontweight','bold');
