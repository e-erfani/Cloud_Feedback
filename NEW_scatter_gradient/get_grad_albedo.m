...
%
      ...
      basins_mask=pacific_mask(infile);
      [nlon,nlat]=size(basins_mask);
      pacific=zeros(nlon,nlat);
      I_pac=find(basins_mask==1);
      pacific(I_pac)=1; clear I_pac
      I_pac=find(pacific<1);    
      pacific(I_pac)=NaN; 
      albedo=albedo_1.*pacific;
      GW=GW_1.*pacific;

      mask=zeros(size(albedo_1));
      % set tropics as 1
      J = find (lat_rho>-8 & lat_rho<8 & lon_rho>130 & lon_rho<280);
      mask(J)=1; clear J
