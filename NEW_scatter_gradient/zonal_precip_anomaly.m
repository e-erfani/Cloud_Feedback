...
cd (address)
  ...
  fname_fcm = {'chey_ctrl_rhminl_PreInd_T31_gx3v7.cam.h0.0471-0570._ANN_climo.nc',...
              'chey_co2_2_rhminl_PreInd_T31_gx3v7.cam.h0.1001-1100._ANN_climo.nc',...
...
%%%%%%%
       fig_name = strcat('zonal_precip_FCM_anomaly');%,num2str(tt));
        fig_dum = figure(1);
...
for j = 6:9
    ...
    ylabel('Precipitation rate anomaly (mm day^-^1)','fontsize',23,'fontweight','bold');
xlim([-90 90])
ylim([-0.7 2.2])
...
%%%%
       fig_name = strcat('zonal_precip_SOM_anomaly');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
ylim([-0.7 2.2])
   hleg1 = legend([h(1) h(2) h(3) h(4) h(5) h(6) h(7) h(8) h(9)],'12',...
    ...
    set(hleg1,'Orientation','horizontal','Location','NorthWest','Fontsize',17)%h_legend-4)
    set(hleg1,'Interpreter','none')%,'box','Off')
set(gca,'Fontsize',20,'linewidth',1.5)
box on
%   eval(['print -r600 -djpeg ', fig_name,'.jpg']);
