...
for i = 1:10
    [var_ZM_fcm,lat] = get_PmE_ZM(char(fname_fcm(:,i))) ;
...
for i = 1:10
    [var_ZM_som,lat] = get_PmE_ZM(char(fname_som(:,i))) ;
...
%%%%%%%
       fig_name = strcat('zonal_P_E_FCM_anomaly_1800');%,num2str(tt));
        fig_dum = figure(1);
...
ylim([-1.5 2])
   hleg1 = legend([h(1) h(2) h(3) h(4) h(5) h(6) h(7) h(8) h(9)],'2',...
    ...
    set(hleg1,'Location','southEast','Fontsize',13)%h_legend-4)
...
%%%%
       fig_name = strcat('zonal_P_E_SOM_anomaly');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
