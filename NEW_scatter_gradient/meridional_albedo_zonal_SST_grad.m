...
%%%% meridional albedo
for i = 1:9
    grad_albedo = get_grad_albedo_zm(char(fname_fcm(:,i)));
...
%%% graident albedo
for i = 1:9
    grad_albedo2 = get_grad_albedo_zm(char(fname_som(:,i)));
    grad_albedo_som(i) = grad_albedo2 ;
end

%%%%%%%
       fig_name = strcat('merid_albedo_zon_SST_grad');%,num2str(tt));
        fig_dum = figure(1);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.1,0.1,11,11]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
      
    h1=plot(grad_albedo_fcm,dSST_fcm,'o','markers',9); hold on
    set(h1,'MarkerEdgeColor','b','MarkerFaceColor','b','LineWidth',4)

    h2=plot(grad_albedo_som,dSST_som,'o','markers',9); hold on
