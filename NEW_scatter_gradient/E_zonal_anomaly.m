...
for i = 1:10
    [var_ZM_fcm,lat] = get_E_ZM(char(fname_fcm(:,i))) ;
...
for i = 1:10
    [var_ZM_som,lat] = get_E_ZM(char(fname_som(:,i))) ;
...
%%%%%%%
       fig_name = strcat('zonal_E_FCM_anomaly_1800');%,num2str(tt));
        fig_dum = figure(1);
...
for j = 6:9
    ...
    ylabel('(P - E) anomaly (mm day^-^1)','fontsize',23,'fontweight','bold');
xlim([-90 90])
ylim([-0.2 1.2])
   hleg1 = legend([h(1) h(2) h(3) h(4) h(5) h(6) h(7) h(8) h(9)],'2',...
         '3','4','5','6','7','8','9','10');% ,'SOM');
    set(hleg1,'Location','best','Fontsize',13)%h_legend-4)
...
%%%%
       fig_name = strcat('zonal_E_SOM_anomaly');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
ylim([-0.2 1.2])
   hleg1 = legend([h(1) h(2) h(3) h(4) h(5) h(6) h(7) h(8) h(9)],'12',...
    ...
    set(hleg1,'Location','bestoutside','Fontsize',13)%h_legend-4)
    set(hleg1,'Interpreter','none')%,'box','Off')
set(gca,'Fontsize',20,'linewidth',1.5)
box on
%   eval(['print -r600 -djpeg ', fig_name,'.jpg']);
