...
for i = 1:9
  ...
  %   TS(-TS > 1.0E+30) = NaN; PS(-PS > 1.0E+30) = NaN;       
     ps=100000; % Pa
     epsilon=0.622; % ration of mass or vapour to dry air = rdgas/rvgas
    TS_c = TS - 273.15 ; 
 %   es = 1000.0 .* 0.6112 .* exp(17.67 .* TS_c ./ (TS_c + 243.5)) ;
     es= 661.*exp((17.27.*(TS - 273.15))./(237.3+(TS-273.15))) ; 
    qs=epsilon.*es./(ps-(1-epsilon).*es); % kg/kg
     ...
     % set 1st region in tropical Pacific to 2
      ...
      TS_1=nansum(TS(B).*area(B))./nansum(area(B));
      clear B mask
     % set 2nd region in tropical Pacific to 2
      ...
      TS_2=nansum(TS(B).*area(B))./nansum(area(B));
      clear B mask

      d_qs(i) = qs_1 - qs_2;
      TS_trop(i) = (TS_1 + TS_2) ./ 2 - 273.15 ;
end
%    d_qs_mean = nanmean(d_qs);
    d_qs_mean = d_qs(5);

    TT = (26.8203+273.15):0.01:(32.7057+273.15); 
    es_es=661.*exp((17.27.*(TT - 273.15))./(237.3+(TT-273.15))) ; % TS in Kelvin
    des_dT = es_es .* Lv ./ (Rv .* TT .^ 2.0) ;
    dqs_dT = epsilon .* des_dT ./ (ps-(1-epsilon).* es_es) + ...
        epsilon .* es_es .* (ps - (1-epsilon) .* es_es) .^ (-2) .* (1-epsilon) .* des_dT;
     %dqs_dT = (0.622 ./ PS) .* des_dT ;

    scaling = d_qs_mean .* dqs_dT .^ (-1); 
    
%   TS_trop = [26.820337	26.926233	27.049036	27.253260	27.540613	28.082178...
%       30.017572	31.842005	32.705711];  
  sst_som2 = 17.74 : 0.01353140916 : 25.70 ;
%%%%%%%
       fig_name = strcat('zonal_grad_vs_GSST_scaling_EXACT_smooth_tropical_ctrl');%,num2str(tt));
        fig_dum = figure(1);
  ...
  %  h3 = plot(TT-273.15- 6,scaling,'color',purple,'LineWidth',3); hold on
  %  h3 = plot(TT-273.15- 7.5,scaling,'color',purple,'LineWidth',3); hold on
    h3 = plot(sst_som2,scaling,'color',purple,'LineWidth',3); hold on
   ...
   hleg1 = legend([h1 h2 h3],'FCM','SOM', 'scaling estimate');
