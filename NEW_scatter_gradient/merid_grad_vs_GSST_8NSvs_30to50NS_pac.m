...
for i = 1:10
    fname = aa(i,1).name;
    grad_SST = get_north_south_grad_8NS_vs_30to50NS_pac(fname);
...
elseif (tt==10);load(aa(tt,1).name); sst = sst_CO_sl_ps_0_2; end
 ...
 sst = double(sst');
     grad_SST_SOM = get_north_south_grad_8NS_vs_30to50NS_pac_SOM(sst,lat_msh_CAM,lon_msh_CAM,GW);
...
%%%%%%%
       fig_name = strcat('merid_grad_vs_GSST_8NSvs_30to50NS_pac');%,num2str(tt));
        fig_dum = figure(1);
