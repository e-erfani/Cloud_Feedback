...
%%%%%%%
       fig_name = strcat('zonal_E_FCM');%,num2str(tt));
        fig_dum = figure(1);
...
for j = 6:9
    ...
    ylabel('P - E (mm day^-^1)','fontsize',23,'fontweight','bold');
xlim([-90 90])
ylim([0 5.0])
   hleg1 = legend([h(1) h(2) h(3) h(4) h(5) h(6) h(7) h(8) h(9)],'2',...
         '3','4','5','6','7','8','9','10');% ,'SOM');
    set(hleg1,'Location','best','Fontsize',16)%h_legend-4)
...
%%%%
       fig_name = strcat('zonal_E_SOM');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
