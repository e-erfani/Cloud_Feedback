...
for i = 1:10
    [u,v,p,lat,lon]=get_850winds(char(fname_fcm(:,i)));
...
for i = 1:10
    [u,v,p,lat,lon]=get_850winds(char(fname_som(:,i)));
...
%%%%%%%
       fig_name = strcat('zonal_U_850_FCM_anomaly_1800');%,num2str(tt));
        fig_dum = figure(1);
...
h(1)=plot(lat(idx),squeeze(ZM_fcm_anom(idx,1)),'color',cl(1),'linewidth',2) ; hold on;
for j = 2:5
    if (j==3)
      h(j)=plot(lat(idx),squeeze(ZM_fcm_anom(idx,j)),'--','color',purple,'linewidth',2) ; hold on;        
    else
      h(j)=plot(lat(idx),squeeze(ZM_fcm_anom(idx,j)),'--','color',cl(j),'linewidth',2) ; hold on;
    end
end
for j = 6:9
    if (j==8)
    ...
    ylabel('850 hPa zonal wind anomaly (m s^-^1)','fontsize',23,'fontweight','bold');
xlim([-90 90])
ylim([-1.6 1.1])
set(gca,'Fontsize',20,'linewidth',1.5)
   hleg1 = legend([h(1) h(2) h(3) h(4) h(5) h(6) h(7) h(8) h(9)],'2',...
         '3','4','5','6','7','8','9','10');% ,'SOM');
    set(hleg1,'Location','southwest','Fontsize',17)%h_legend-4)
...
%%%%
       fig_name = strcat('zonal_U_850_SOM_anomaly');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
h(1)=plot(lat(idx),squeeze(ZM_som_anom(idx,1)),'color',cl(1),'linewidth',2) ; hold on;
for j = 2:5
    if (j==3)
      h(j)=plot(lat(idx),squeeze(ZM_som_anom(idx,j)),'--','color',purple,'linewidth',2) ; hold on;        
    else
      h(j)=plot(lat(idx),squeeze(ZM_som_anom(idx,j)),'--','color',cl(j),'linewidth',2) ; hold on;
    end
end
for j = 6:9
    if (j==8)
