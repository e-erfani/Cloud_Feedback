...
%%%%%%%
       fig_name = strcat('zonal_SST_FCM');%,num2str(tt));
        fig_dum = figure(1);
...
h(1)=plot(lat_ZM,squeeze(SST_ZM_fcm(:,1)),'color',cl(1),'linewidth',2) ; hold on;
for j = 2:5
    if (j==3)
      h(j)=plot(lat_ZM,squeeze(SST_ZM_fcm(:,j)),'--','color',purple,'linewidth',2) ; hold on;        
    else
      h(j)=plot(lat_ZM,squeeze(SST_ZM_fcm(:,j)),'--','color',cl(j),'linewidth',2) ; hold on;
    end
end
for j = 6:9
    if (j==8)
...
%%%%
       fig_name = strcat('zonal_SST_SOM');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
...
h(1)=plot(lat_ZM_SOM,squeeze(SST_ZM_SOM(:,1)),'color',cl(1),'linewidth',2) ; hold on;
for j = 2:5
    if (j==3)
      h(j)=plot(lat_ZM_SOM,squeeze(SST_ZM_SOM(:,j)),'--','color',purple,'linewidth',2) ; hold on;        
    else
      h(j)=plot(lat_ZM_SOM,squeeze(SST_ZM_SOM(:,j)),'--','color',cl(j),'linewidth',2) ; hold on;
    end
end
for j = 6:9
    if (j==8)
