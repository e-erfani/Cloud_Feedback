...
var_SOM = dSST_m ./ ECS_SOM ;
       ...
       fig_name = strcat('diff_zonal_grad_vs_GSST_NORMALIZED_NEW');%,num2str(tt));
        fig_dum = figure(1);
   ...
   y = ylabel('Normalized chane of \DeltaT_z_o_n_a_l','fontsize',23,'fontweight','bold');
...
%%%%%%%
       fig_name = strcat('diff_zonal_grad_vs_c_NORMALIZED_NEW');%,num2str(tt));
        fig_dum = figure(2);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
