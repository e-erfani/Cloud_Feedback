...
for i = 1:9
  ...
  %   TS(-TS > 1.0E+30) = NaN; PS(-PS > 1.0E+30) = NaN;       
    ...
    TS_c = TS - 273.15 ; 
    es=661.*exp((17.27.*TS_c)./(237.3+TS_c)) ; % TS in Kelvin
    qs=epsilon.*es./(ps-(1-epsilon).*es); % kg/kg
        
    des_dT = es .* Lv ./ (Rv .* TS .^ 2.0) ;
    dqs_dT = epsilon .* des_dT ./ (ps-(1-epsilon).* es) + ...
        epsilon .* es .* (ps - (1-epsilon) .* es) .^ (-2) .* (1-epsilon) .* des_dT;
...
sst_som2 = [17.74	17.86	18      18.22	18.5     20.99	22.98	24.97	25.71...
           16.38	16.37	16.37	16.38	16.38	16.42	16.48	16.64	16.81];
xx = 16.0:0.01:26 ;
p = polyfit(sst_som2(1:9),scaling,2);
f = polyval(p,xx);
    
%%%%%%%
       fig_name = strcat('zonal_grad_vs_GSST_scaling_EXACT_fit');%,num2str(tt));
        fig_dum = figure(1);
   ...
   % h3 = plot(sst_som2(1:9),scaling,'color',purple,'LineWidth',3); hold on
    h3 = plot(xx,f,'color',purple,'LineWidth',3); hold on;
