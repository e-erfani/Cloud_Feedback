...
sst_fcm = [18.25	17.84	17.89	18.03	18.17	18.54	19.13	21.47...
    23.73	16.6	16.53	16.51	16.53	16.53	16.6	16.61	16.59	16.6];
...
for i = 1:10
    fname = aa(i,1).name;
    [T1(i),T1_area(i),T1_dx(i)] = get_SST1_Tuo(fname); 
    [T2(i),T2_area(i),T2_dx(i)] = get_SST2_Tuo(fname); 
    [T3(i),T3_area(i)] = get_SST3_Tuo_25to65NS(fname);
 end

Pac_meri_grad_UO_fcm_temp=((T1+T2)./2)-T3;
Pac_meri_grad_UO_fcm_temp(2) = [];
PPac_meri_grad_UO_fcm = Pac_meri_grad_UO_fcm_temp ;
nn = 2;
for i = 5:-1:2
  PPac_meri_grad_UO_fcm(nn) = Pac_meri_grad_UO_fcm_temp(i) ;
  nn = nn + 1 ;
end
%%%%
...
elseif (tt==10);load(aa2(tt,1).name); sst = sst_CO_sl_ps_0_2; end
 ...
 sst = double(sst);
    ...
    T1_S(tt) = get_SST1_Tuo_SOM(sst, lat_msh_CAM, lon_msh_CAM, GW); 
    T2_S(tt) = get_SST2_Tuo_SOM(sst, lat_msh_CAM, lon_msh_CAM, GW);  
    T3_S(tt)= get_SST3_Tuo_25to65NS_SOM(sst, lat_msh_CAM, lon_msh_CAM, GW); 
 
end     

dSST_m_temp=((T1_S+T2_S)./2)-T3_S;
dSST_m_temp(2) = [];
nn = 2 ;
dSST_m = dSST_m_temp ;
for i = 5:-1:2
...
%%%%%%%
       fig_name = strcat('meridional_grad_vs_global_region_Tuo_');%,num2str(tt));
        fig_dum = figure(1);
      set(fig_dum, 'name', fig_name,'numbertitle','on');
      set(fig_dum,'units','inches','position',[0.5,1,11,8]);%,'PaperOrientation','landscape');
      set(fig_dum,'paperpositionmode','auto');
      
    h1=plot(sst_fcm(1:9),PPac_meri_grad_UO_fcm,'o','markers',9); hold on
