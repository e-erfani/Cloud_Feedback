...
[lat_msh,lon_msh] = meshgrid(latitude,longitude);
      I=length(longitude);
      GW=repmat(gw,[1 I])';
      precc = ncread(filename1,'PRECC');
      precl = ncread(filename1,'PRECL');
      cldlow_0_15 = (precc + precl) .* 1000.*60.*60.*24;
...
cd ../PreInd_chey_contr   
    ...
    filename1=aa1(tt1,1).name;
      precc = ncread(filename1,'PRECC');
      precl = ncread(filename1,'PRECL');
      cldlow_ctrl = (precc + precl) .* 1000.*60.*60.*24;
...
[Z_adjust4,refvec_bias4] = geoloc2grid(lat,lon,var4, cellsize);   
 ...
 lonlim = [1 360]; 
       ...
       fig_name = strcat('Diff_precip_CESM_ctrl_PAPER_FCM');
        fig_dum = figure(1);
