...
[lat_msh,lon_msh] = meshgrid(latitude,longitude);
      I=length(longitude);
      GW=repmat(gw,[1 I])';
     cldlow_0_15 = ncread(filename1,'CLDLOW') .* 100; % low cloud amount
...
cd ../PreInd_chey_contr   
    aa1=dir('*._ANN_climo.nc');
    filename1=aa1(tt1,1).name;
   cldlow_ctrl=ncread(filename1,'CLDLOW') .* 100; % low cloud amount
%%%%    
cd ../lay_strat_0_1_co2_2_CHEY_PreIn   
    aa1=dir('*._ANN_climo.nc');
    filename1=aa1(tt1,1).name;
   cldlow_0_1=ncread(filename1,'CLDLOW') .* 100; % low cloud amount
%%%%    
...
cd ../chey_co2_2_rhminl_PreIn   
    aa1=dir('*._ANN_climo.nc');
    filename1=aa1(tt1,1).name;
   cldlow_CO2_2=ncread(filename1,'CLDLOW') .* 100; % low cloud amount   
